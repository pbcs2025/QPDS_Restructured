...
function MBAPapers() {
  ...
  // Load and persist Sent for Print and Archived for MBA
  useEffect(() => {
    try {
      const s = JSON.parse(localStorage.getItem('papersSentForPrint_MBA') || '[]');
      const a = JSON.parse(localStorage.getItem('archivedPapers_MBA') || '[]');
  ...
  useEffect(() => {
    try { localStorage.setItem('papersSentForPrint_MBA', JSON.stringify(sentForPrint)); } catch (_) {}
  }, [sentForPrint]);
  useEffect(() => {
  ...
  // Validated DOCX download and mark as sent for print
  const handlePrint = async (p) => {
    try {
      ...
      setSentForPrint(prev => [{...p, degree: 'MBA', sentForPrintAt: new Date().toISOString()}, ...prev.filter(x => !(x.subject_code===p.subject_code && x.semester===p.semester))]);
    } catch (err) {
  ...
  const handleArchive = (p) => {
    setArchived(prev => [{...p, degree: 'MBA', archivedAt: new Date().toISOString()}, ...prev.filter(x => !(x.subject_code===p.subject_code && x.semester===p.semester))]);
  };
  ...
  return (
    <div>
      ...
      {/* Sent for Print Section */}
      {showSentForPrint ? (
        <div className="table-wrapper">
          <h3 style={{ marginBottom: '12px', color: '#495057' }}>Papers Sent for Print (MBA)</h3>
        ...
        <div className="table-wrapper">
          <h3 style={{ marginBottom: '12px', color: '#495057' }}>Archived Papers (MBA)</h3>
