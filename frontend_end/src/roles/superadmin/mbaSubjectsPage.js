...
function MBASubjectsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_BASE}/mbasubjects`, {
        method: "POST",
        headers: { 
  ...
  return (
    <div className="subjects-container">
      ...
      {!selectedDept && (
        <>
          <h1>MBA Departments</h1>
          <div className="departments-grid">
            {departments.length === 0 && <p>No MBA departments found.</p>}
            {departments.map((dept) => {
...
export default MBASubjectsPage;
