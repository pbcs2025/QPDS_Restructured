...
const ViewAnalytics = () => {
  ...
  const handleConfirm = () => {
    ...
    const fetchAnalytics = async () => {
      try {
        // 1) Get all assignments (papers sent to setters)
        let sentCount = 0;
        try {
          // First get the subject details to ensure we have the correct department and semester
          const subjectDetails = allSubjects.find(s => 
          ...
          if (subjectDetails) {
            // Get all assignments for this subject code
            const assignmentsRes = await axios.get(`${API_BASE}/assignments/${encodeURIComponent(subject)}`);
            
            // The API returns an array of assignments for this subject code
            // Each assignment has a user object with department info
            const assignments = Array.isArray(assignmentsRes.data) ? assignmentsRes.data : [];
            
            // Count unique faculty assignments
            const uniqueFaculty = new Set(assignments.map(a => a.email));
            sentCount = uniqueFaculty.size;
        ...
        try {
          ...
          submittedPapers = (submittedRes.data?.papers || []).filter(paper => 
            paper.department === dept && 
            String(paper.semester) === String(sem) &&
            paper.subject_code === subject
          );
        } catch (err) {
          console.error('Failed to fetch submitted papers', err?.response?.status, err?.message);
          submittedPapers = [];
        }

        // 3) Calculate counts
        let receivedCount = submittedPapers.length;
        // Ensure received count doesn't exceed sent count
        receivedCount = Math.min(receivedCount, sentCount);
        const pendingCount = Math.max(0, sentCount - receivedCount);
        const rejectedCount = 0; // Not tracking rejected papers
        
        console.log('Calculated counts:', {
          sent: sentCount,
          received: receivedCount,
          pending: pendingCount,
          note: receivedCount === submittedPapers.length ? 'No adjustment needed' : 'Adjusted received count to match sent count'
        });
        ...
        console.log('Final chart data:', dynamicData);
  ...
  // Tooltip that shows only the hovered bar's value
